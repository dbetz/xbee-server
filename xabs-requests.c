        ...
        if (match("ADC"))
        {
            ...
            if(num > -1 && num < 4) {
                ...
                if(len > 0) {
                    ...
                    strcat(resp, "");
                    //puts(resp);
                    http_send_response(sock, (uint8_t *)resp, strlen(resp) - 1);
                    return;
                }
            }
            // good exit point above sends data back
            //
            http_send_response(sock, (uint8_t *)CANNED_RESPONSE, sizeof(CANNED_RESPONSE) - 1);
        }
...
static void handle_xping_request(Socket_t *sock, int phase)
{
    if (phase == HP_CONTENT) {
        DIRA &= ~(3 << 26);
        servo_stop();
        http_send_response(sock, (uint8_t *)XPING_RESPONSE, sizeof(XPING_RESPONSE) - 1);
    }
}
